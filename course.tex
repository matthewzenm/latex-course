...
\begin{frame}[fragile]
    ...
    \textbf{提示2}: 课下你可能会找到某些告诉你使用\Verb|CJK|包或者\Verb|cctart|文档类输入中文的资料, 这是\textbf{过时的}, 一定不要学.
\end{frame}
...
\begin{frame}[fragile]
    ...
    以及
    \[\operatorname{Vol}(B_r(p))=\omega_nr^n\left(1-\frac{\operatorname{Scal}(p)}{6(n+2)}+O(r^3)\right)\quad(r\to 0).\]
...
\begin{frame}[fragile]
    ...
    \end{lstlisting}
    注意使用\Verb|fontspec|包的话必须使用XeLaTeX编译.\\
